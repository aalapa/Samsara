...
@Database(entities = [Persona::class, Task::class, Comment::class, PersonaStatistics::class, Tag::class, PersonaTag::class, PersonaOpenEvent::class, DailyCompletion::class], version = 25, exportSchema = false)
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_13_14 = object : androidx.room.migration.Migration(13, 14) {
            override fun migrate(database: androidx.sqlite.db.SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE tasks ADD COLUMN endDate INTEGER DEFAULT NULL")
            }
        }
        ...
        private val MIGRATION_18_19 = object : androidx.room.migration.Migration(18, 19) {
            override fun migrate(database: androidx.sqlite.db.SupportSQLiteDatabase) {
                // Add isAvoidTask column for streak/avoid tasks (completing = failure)
                database.execSQL("ALTER TABLE tasks ADD COLUMN isAvoidTask INTEGER NOT NULL DEFAULT 0")
            }
        }
        ...
        private val MIGRATION_19_20 = object : androidx.room.migration.Migration(19, 20) {
            override fun migrate(database: androidx.sqlite.db.SupportSQLiteDatabase) {
                // Add isFocused column to personas table for Focus view filtering
                database.execSQL("ALTER TABLE personas ADD COLUMN isFocused INTEGER NOT NULL DEFAULT 0")
            }
        }

        private val MIGRATION_20_21 = object : androidx.room.migration.Migration(20, 21) {
            override fun migrate(database: androidx.sqlite.db.SupportSQLiteDatabase) {
                // Add isChakra column to personas table for Chakra view filtering
                database.execSQL("ALTER TABLE personas ADD COLUMN isChakra INTEGER NOT NULL DEFAULT 0")
            }
        }
        ...
        private val MIGRATION_23_24 = object : androidx.room.migration.Migration(23, 24) {
            override fun migrate(database: androidx.sqlite.db.SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE personas ADD COLUMN isRoutine INTEGER NOT NULL DEFAULT 0")
                database.execSQL("ALTER TABLE tasks ADD COLUMN timeChunk TEXT NOT NULL DEFAULT 'MORNING'")
                database.execSQL("ALTER TABLE tasks ADD COLUMN scheduledDays INTEGER NOT NULL DEFAULT 0")
                database.execSQL("""
                ...
                """.trimIndent())
                database.execSQL("CREATE INDEX IF NOT EXISTS index_daily_completions_taskId ON daily_completions(taskId)")
                database.execSQL("CREATE INDEX IF NOT EXISTS index_daily_completions_completedDate ON daily_completions(completedDate)")
                database.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_daily_completions_task_date ON daily_completions(taskId, completedDate)")
            }
        }

        private val MIGRATION_24_25 = object : androidx.room.migration.Migration(24, 25) {
            override fun migrate(database: androidx.sqlite.db.SupportSQLiteDatabase) {
                database.execSQL("ALTER TABLE personas ADD COLUMN isSuspended INTEGER NOT NULL DEFAULT 0")
            }
        }
